...
1. 検索結果の件名（subject）から通知内容を抽出する
   - 件名パターン例: `Issue [EXAMPLE-893] タイトル added to the プロジェクト名...`
   - 件名パターン例: `Linear auto-closed EXAMPLE-207, and changed the status to Canceled`
   - 件名パターン例: `N unread notifications on Example Workspace`（まとめ通知、詳細なし）
...
[個別通知があればissue IDをwikiリンク `[[EXAMPLE-XXX]]` にして、内容を散文で記述する。]
...
- issue IDは `[[EXAMPLE-XXX]]` のwikiリンクにする
