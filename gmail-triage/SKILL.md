---
name: gmail-triage
description: Triage Gmail inbox by archiving noise emails (Google Meet, Calendar invitations, <対象メーリングリスト> without action needed) and surfacing actionable items. Use as standalone or as part of /today workflow.
---
...
以下のカテゴリを検索する。カテゴリCはユーザーが対象メーリングリストのアドレスを指定した場合だけ実行し、`<対象メーリングリスト>` をそのアドレスに置き換える。指定がなければカテゴリCはスキップする。
...
# <対象メーリングリスト> からのメール
mcp__claude_ai_Gmail__search_threads with query: "in:inbox to:<対象メーリングリスト>"
```
...
- 判断に迷うメール（<対象メーリングリスト>）はアーカイブせずユーザーに判断を委ねる
