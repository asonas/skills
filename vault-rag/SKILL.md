...
- `lex:` にはリポジトリ名、ツール名、`EXAMPLE-148` のような Issue ID、日本語の固有名詞を入れる。vault は固有名詞が多いので lex が効く
...
`bench/README.md` に検索品質の測定手順がある。実際の質問・正解ファイル・測定結果はVaultの `.agent-state/vault-rag/` に保存し、配布スキルには書き込まない。qmdの設定・インデックス・コレクション構成を変えたら、同じ質問セットで前後を測る。実データが未作成なら、その旨を報告して測定を保留する。

`wiki/log*.md`・`wiki/index.md`・`wiki/deferred.md` は qmd のインデックスから除外されている（`~/.config/qmd/index.yml` の ignore）。これらは wiki 運用のブックキーピングで固有名詞の密度が高く、リランキングの候補枠を占有して本来の wiki ページを押しのけていた。除外の効果はローカルの質問セットで検証する。
